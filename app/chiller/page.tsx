...
import { submitLog, loadLogs, ChillerLog } from "../lib/logData";
import { PRODUCTS } from "../lib/products";

const MODULES = [
  { id: "CHL-001", name: "Chilled Water & Condensate Logs", tags: ["Logs", "active", "baselined"], location: "Mechanical Room", docs: ["Log Sheet 1–3", "CS Optimization"],          file: "chiller/logs/NS-CHLLR-PRM-LOG001.pdf" },
  { id: "CHL-002", name: "Cooling Tower Performance",       tags: ["Guide", "active"],             location: "Rooftop / Tower",  docs: ["CT Guide 2 & 3", "CT Checklist"],           file: "chiller/guide/NS-CHLLR-PRM-CT002.pdf" },
  { id: "CHL-003", name: "Refrigerant Temp Monitoring",     tags: ["Safety", "active"],            location: "Mechanical Room",  docs: ["Refrigerant Guide", "Maintenance Checklist"], file: "chiller/guide/NS-CHLLR-PRM-REF001.pdf" },
];

const TABS = ["Overview", "Log Data", "Resources"] as const;
type Tab = typeof TABS[number];

const EMPTY: ChillerLog = {
  timestamp: "", date: "", equipmentId: "CHL-001", chillerName: "",
  supplyTemp: "", returnTemp: "", condenserTemp: "", refrigerantPressure: "",
  compressorAmps: "", coolingTowerTemp: "", flowRate: "", techName: "", notes: "",
};
...
export default function ChillerPage() {
  ...
  const now = new Date();
  const [form, setForm] = useState<ChillerLog>({ ...EMPTY, timestamp: now.toISOString().slice(0, 16), date: now.toISOString().slice(0, 10) });
  const [logs, setLogs] = useState<ChillerLog[]>([]);
  const [submitting, setSubmitting] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => { setLogs(loadLogs<ChillerLog>("chiller")); }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    await submitLog("chiller", { ...form, techName: form.techName || user.name });
    const updated = loadLogs<ChillerLog>("chiller");
    setLogs(updated);
    const n = new Date();
  ...
  const setF = (k: keyof ChillerLog, v: string) => setForm((f) => ({ ...f, [k]: v }));
  ...
  const chartData = [...logs].reverse().slice(-20).map((l, i) => ({
    ...
    "Condenser":   parseFloat(l.condenserTemp) || null,
  }));
      ...
      <div className="px-7 pt-7 pb-5 border-b" style={{ borderColor: "rgba(56,189,248,0.06)" }}>
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
            style={{ background: `rgba(${ACCENT_RGB},0.08)`, border: `1px solid rgba(${ACCENT_RGB},0.18)` }}>❄️</div>
          <div>
            <h1 className="text-xl font-bold" style={{ color: ACCENT }}>Chiller Intelligence</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Return Temp",  val: last?.returnTemp,         unit: "°F" },
            { label: "Condenser",    val: last?.condenserTemp,      unit: "°F" },
            { label: "Comp. Amps",   val: last?.compressorAmps,     unit: "A"  },
          ].map((t) => (
            <div key={t.label} className="rounded-xl p-4"
              ...
              <p className="text-[10px] text-gray-700 mt-0.5">{t.unit}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-7 py-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex-1 relative">
                ...
                <input type="text" placeholder="Search modules…" value={search} onChange={(e) => setSearch(e.target.value)}
                  className="w-full rounded-xl pl-9 pr-4 py-2 text-sm text-gray-400 placeholder-gray-700 outline-none"
        ...
        {tab === "Log Data" && (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid rgba(${ACCENT_RGB},0.1)` }}>
              <h2 className="font-bold text-white mb-1 text-sm">Submit Chiller Reading</h2>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="date" value={form.date} onChange={(e) => setF("date", e.target.value)}
                      className="w-full rounded-lg px-3 py-2 text-sm text-gray-300 outline-none"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }} />
                  </div>
                  <div>
                    <label className="block text-[10px] text-gray-600 uppercase tracking-wide mb-1">Equipment ID</label>
                    <input type="text" value={form.equipmentId} onChange={(e) => setF("equipmentId", e.target.value)} placeholder="CHL-001"
                      className="w-full rounded-lg px-3 py-2 text-sm text-gray-300 placeholder-gray-700 outline-none"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[10px] text-gray-600 uppercase tracking-wide mb-1">Chiller Name / Location</label>
                    <input type="text" value={form.chillerName} onChange={(e) => setF("chillerName", e.target.value)} placeholder="Centrifugal Chiller 1"
                      className="w-full rounded-lg px-3 py-2 text-sm text-gray-300 placeholder-gray-700 outline-none"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }} />
                  </div>
                  {([
                    ["supplyTemp",          "Supply Temp (°F)",   "44"],
                    ["returnTemp",          "Return Temp (°F)",   "54"],
                    ["condenserTemp",       "Condenser Temp (°F)","95"],
                    ["refrigerantPressure", "Refrigerant Press.", "185"],
                    ["compressorAmps",      "Compressor Amps",    "210"],
                    ["coolingTowerTemp",    "Cooling Tower (°F)", "85"],
                    ["flowRate",            "Flow Rate (GPM)",    "800"],
                    ["techName",            "Tech Name",          user.name],
                  ] as [keyof ChillerLog, string, string][]).map(([field, label, placeholder]) => (
                    <div key={field}>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea value={form.notes} onChange={(e) => setF("notes", e.target.value)} rows={2} placeholder="Observations…"
                      className="w-full rounded-lg px-3 py-2 text-sm text-gray-300 placeholder-gray-700 outline-none resize-none"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }} />
                  </div>
                </div>
                ...
                <div className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="Return Temp" stroke="#00FFE1" strokeWidth={2} dot={false} connectNulls />
                      <Line type="monotone" dataKey="Condenser"   stroke="#fbbf24" strokeWidth={2} dot={false} connectNulls />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
                  ...
                  {logs.slice(0, 8).map((log, i) => (
                    <div key={i} className="flex items-center justify-between px-5 py-3 text-xs"
                      ...
                      <div className="flex gap-4 text-gray-600">
                        {log.supplyTemp && <span><span style={{ color: ACCENT }}>{log.supplyTemp}°F</span> supply</span>}
                        {log.condenserTemp && <span>{log.condenserTemp}°F cond.</span>}
                        {log.compressorAmps && <span>{log.compressorAmps}A</span>}
                      </div>
        ...
        {tab === "Resources" && (
          <div className="max-w-3xl">
            <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
              {PRODUCTS.chiller_intelligence_looker.documents.map((doc, i, arr) => (
